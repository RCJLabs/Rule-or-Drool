...
/** A run the rival did nothing in. */
const QUIET: RivalTrace = { top: false, cards: 0, honest: 0, stood: 0, wentOver: false, won: false };
...
describe("bands, votes and looks", () => {
  it("reads a bot's runs, sent as records, exactly as that bot's own", () => {
    for (const bot of ["mixed", "greedy"] as BotName[]) {
      ...
      for (const rows of [r.bands, r.votes, r.looks, r.turns, r.rival] as { label: string }[][]) {
  ...
  it("splits the votes by whether an honest count would have won them, and the cheated ones by the meters", () => {
    ...
    const people: Trace[] = [
      { looks: [0, 1, 1, 0], choices: 0, turns: [], votes: [v(true, true, false), v(false, true, true), v(false, true, false)], rival: QUIET },
      { looks: [0, -1, -1, -2, -2, -2], choices: 0, turns: [], votes: [v(false, false, true), v(true, false, false)], rival: QUIET },
    ];
    ...
    const clean = buildReport(library, gather([source("p", [made(1, [["x", 1000]])])]), new Map(), OPTS, { people: [{ looks: [0], choices: 0, turns: [], votes: [v(true, true, false)], rival: QUIET }], bots: new Map() });
  ...
  it("says how often people turned from the honest side, whether the screen showed a reason, and how near an edge", () => {
    const people: Trace[] = [
      { looks: [0], votes: [], choices: 6, turns: [30, 10], rival: QUIET },
      { looks: [0], votes: [], choices: 4, turns: [20], rival: QUIET },
    ];
    ...
    const steady = buildReport(library, g, new Map(), OPTS, { people: [{ looks: [0], votes: [], choices: 3, turns: [], rival: QUIET }], bots: new Map() });
  ...
  // BACKLOG-10 phase 65: what the rival did, for people beside the bots.
  it("says how often the rival got to the top rung, what they did, how it was answered and whether they won", () => {
    const people: Trace[] = [
      { looks: [0], votes: [], choices: 0, turns: [], rival: { top: true, cards: 3, honest: 2, stood: 1, wentOver: true, won: false } },
      { looks: [0], votes: [], choices: 0, turns: [], rival: { top: true, cards: 1, honest: 0, stood: 1, wentOver: false, won: true } },
      { looks: [0], votes: [], choices: 0, turns: [], rival: QUIET },
      { looks: [0], votes: [], choices: 0, turns: [], rival: QUIET },
    ];
    const g = gather([source("p", [made(1, [["x", 1000]])])]);
    const r = buildReport(library, g, new Map(), OPTS, { people, bots: new Map() });
    expect(r.rival).toEqual([{ label: "people", runs: 4, top: 0.5, cards: 1, honest: 0.5, wentOver: 0.25, stood: 0.5, won: 0.25 }]);
    const text = formatReport(r);
    expect(text).toContain("== the rival, on the same runs ==");
    expect(text).toMatch(/^people\s+4\s+50\.0%\s+1\.00\s+50\.0%\s+25\.0%\s+50\.0%\s+25\.0%$/m);
    // A rival who did nothing reads as nothing taken, not as a share of nothing.
    const quiet = buildReport(library, g, new Map(), OPTS, { people: [people[2]!], bots: new Map() });
    expect(quiet.rival[0]).toMatchObject({ top: 0, cards: 0, wentOver: 0, won: 0 });
    expect(quiet.rival[0]!.honest).toBeNaN();
  });
  ...
  it("sets people told how the count stood beside people who were not, when there are both", () => {
    ...
    const told: Trace = { looks: [0], choices: 0, turns: [], votes: [v(true, true), v(true, true)], line: true, rival: QUIET };
    const before: Trace = { looks: [0], choices: 0, turns: [], votes: [v(false, true), v(true, true)], line: false, rival: QUIET };
    const g = gather([source("p", [made(1, [["x", 1000]])])]);
